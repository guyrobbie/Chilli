...
{
	for( int index = 0; index < NSCORE; index++ )
	{
		scores[ index ].score = -1;
	}
}

void Game::InsertScore( HighScore score )
{
	int index;
	for( index = 0; index < NSCORE &&
		scores[ index ].score >= score.score; index++ );

	if( index < NSCORE )
	{
		for( int shuffleIndex = NSCORE - 2; shuffleIndex >= index; shuffleIndex-- )
		{
...
void Game::SaveScoreboard()
{
	...
	for( int index = 0; index < NSCORE 
		&& scores[ index ].score >= 0; index++ )
	{
		fprintf( pFile,"%d.\t%d\t%s\n",(index + 1),scores[ index ].score,scores[ index ].time );
	}
...
void Game::LoadScore()
{
	...
	if( pFile )
	{
		...
		do
		{
			fscanf( pFile,"%d %s",&scores[ index ].score,scores[ index ].time );
			index++;
		} while( !fseek( pFile,5,SEEK_CUR ) && index < NSCORE );
