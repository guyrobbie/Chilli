...
public:
	...
	{
		const int yStart = (unsigned int)max(-yoff,0);
		const int xStart = (unsigned int)max(-xoff, 0);
		const int yEnd = (unsigned int)min(SCREENHEIGHT - yoff,(int)height);
		const int xEnd = (unsigned int)min(SCREENWIDTH - xoff, (int)width);
